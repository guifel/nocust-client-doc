...
| Name       | Type   |                          Description |
| :-------------- | :----- | -----------------------------------: |
| to  | string | Recipient of the transfer, this address needs to have been registered with the payment hub at least once in the past. You can insure that the receipient is registered by using the [isAddressRegisteredWithHub](#lqdmanager-isaddressregisteredwithhub) method|
...
__TransferWalletDataInterface:__
| Parameter       | Type   |                          Description |
| :-------------- | :----- | -----------------------------------: |
...
Make an on-chain transaction to initiate a withdrawal to remove the funds from the NOCUST smart contract and to get them at the specified address. The amount specified needs [to be available for withdrawal](#lqdmanager-getwithdrawallimit) . The withdrawal will have to be [confirmed](#lqdmanager-withdrawalconfirmation) after a certain period of time (Currently between 36 and 72h).  On the top the gas fee, this function will take an [Ether fee](#lqdmanager-getwithdrawalfee) from the on-chain balance for the hub operator. 
...
Make an on-chain transaction to confirm a withdrawal previously [initialized](#lqdmanager-withdrawalrequest) and effectively transfer the funds.
...
Check weather the payment is on recovery mode (Meaning that the payment lost a challenge or missed to submit a checkpoint). If it returns true the payment hub is no longer in operation, the only operation possible to withdraw your off-chain balance using the [recoverFunds](#lqdmanager-recoverfunds) method. 
